public class B3_HalfTheRed extends World {
    ...
    public void go() {
        ...
        plane.teleport(0, 0);
        plane.showBackGround(); //method that show a picture (file name found on line 12 of run my program
        System.out.println(plane.howMuchRed()); // howMuchRed is a method that tells us the "r" value of the plane's current pixel

        for (int row = 160; row < 218; row = row + 1) {
            for (int col = 130; col < 204; col++) {
                plane.teleport(col, row);

                red = plane.howMuchRed();
                green = plane.howMuchGreen();
                blue = plane.howMuchBlue();
//                plane.teleport(col, 130 - row);
